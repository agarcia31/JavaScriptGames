...
function App() {
  ...
  function placePiece(row, col, length, direction, ships) {
    ...
    for (let i = 0; i < length; i++) {
      if (direction === 'horizontal') {
        if (col + i >= 10 || ships.some(([r, c]) => r === row && c === col + i)) {
          canPlaceShip = false;
          break;
        }
        ship.push([row, col + i]);
      } else {
        if (row + i >= 10 || ships.some(([r, c]) => r === row + i && c === col)) {
          canPlaceShip = false;
          break;
        }
        ship.push([row + i, col]);
  ...
  return (
    <div className="h-screen flex flex-col items-center justify-center bg-gray-200">
        ...
        {board.map((row, rowIndex) =>
          row.map((cell, colIndex) => (
            <div
              ...
              className={`${
                cell === 0 ? 'bg-blue-500' : cell === 1 ? 'bg-gray-400' : cell === 2 ? 'bg-red-500' : 'bg-gray-500'
              } h-full w-full`}
      ...
      {isGameOver && <p className="mt-4 text-2xl font-bold text-green-500">Game Over! You Won!</p>}
    </div>
  );
}
